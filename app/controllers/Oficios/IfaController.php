...
class IfaController extends Template {
	...
	public function index() {
        ...
        ,'c.nombre as caracter','a.nombre as accion','audi.clave','sia_Volantes.extemporaneo','t.estadoProceso')
        	...
        	echo $this->render('/documentos/Ifa/index.twig',[
            'iracs' => $iracs,
            'sesiones'=> $_SESSION,
            'modulo' => 'Irac',
            ]);
	...
	public function create($id,$message, $errors) {
        ...
        echo $this->render('documentos/Ifa/create.twig',[
			'sesiones' => $_SESSION,
			'modulo' => $this->modulo,
			'mensaje' => $message,
			'errors' => $errors,
            ...
            if(empty($errors)){
                if(empty($this->validate($data))){
                    ...
                    if(ApiController::upload_files($id,$max,$files)){
                        ...
                        $app->redirect('/SIA/juridico/Ifa/'.$id);        
                    } else { 
    ...
    public function createDocumentos($id,$message, $errors) {
        $turnados = $this->load_personal($id);
         ...
         echo $this->render('documentos/Ifa/documentos.twig',[
            'sesiones' => $_SESSION,
            'modulo' => $this->modulo,
            'mensaje' => $message,
            'errors' => $errors,
